...
class Ui_LoginWindow(object):
    ...
    def login(self):
        ...
        if loginSuccess:
            ...
            sendDataToHome = Ui_HomeWindow()
            sendDataToHome.receive_id(get_id)
            sendDataToHome.receive_prem(get_premium)
            
            self.goToHomePage()
        else:
            if len(username) == 0:
                QMessageBox.critical(None, "Login fail!", "Username is required")
            elif len(password) == 0:
                QMessageBox.critical(None, "Login fail!", "Password is required")
            elif len(password) > 16 or len(password) < 8:
                QMessageBox.critical(None, "Login fail!", "Password must be more than 8 characters less than 16 characters!")
            else:
                QMessageBox.critical(None, "Login fail!", "Username or password is incorrect!")
    ...
    def forgotPassword(self):
        QMessageBox.critical(self,"Lỗi", "In development")
